@extends('layouts.app')
@section('title', 'Admin Deposit Override')
@section('content')
<div class="dashboard-header">
    <div>
        <h2 style="font-size: 1.8rem; font-weight: 800;">Admin Deposit Override (USD)</h2>
        <p style="color: var(--text-secondary);">Directly credit funds to any target Corox Bank USD account.</p>
    </div>
</div>
...
<div style="background: var(--bg-surface); border: 1px solid var(--navy-border); border-radius: var(--radius-xl); padding: 2.5rem; max-width: 600px;">
    <form action="{{ url('/admin/deposit') }}" method="POST">
        @csrf
        ...
        <button type="submit" class="btn btn-primary btn-lg btn-block">Execute Admin Deposit</button>
    </form>
</div>
@endsection
